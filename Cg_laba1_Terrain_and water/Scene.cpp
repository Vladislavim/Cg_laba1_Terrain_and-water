...
void Scene::HandleMouseClick(int mouseX, int mouseY, int screenWidth, int screenHeight)
{
    ...
    // 1. Берём viewProj и считаем его инверс
    ...
    // 2. Мышь (пиксели) -> NDC [-1..1]
    float ndcX = 2.0f * mouseX / screenWidth - 1.0f;
    float ndcY = 1.0f - 2.0f * mouseY / screenHeight; // инверсия Y

    XMVECTOR nearPt = XMVectorSet(ndcX, ndcY, 0.0f, 1.0f);
    XMVECTOR farPt = XMVectorSet(ndcX, ndcY, 1.0f, 1.0f);

    // 3. Восстанавливаем мировые near/far и направление луча
    ...
    // 4. Идём по лучу вперёд и ищем пересечение с террейном
    float t = 0.0f;
